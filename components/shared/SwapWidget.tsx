...
const SwapWidget = () => {
  ...
  const [selectedSourceToken, setSelectedSourceToken] = useState<
    ERC20Token | NativeToken | undefined
  >(undefined);
  ...
  const [selectedDstAddress, setDstAddress] = useState("");
  ...
  useEffect(() => {}, [txHash, wrappedTokensReleased, tokensReleased]);
  ...
  const handleSubmitAndBridge = async () => {
    ...
    const ethersProvider = new ethers.BrowserProvider(
      isOnLukso ? window.lukso : wallet.provider
    );
    ...
    try {
      if (selectedSourceToken.isNative) {
        const wrappedTx = await wrapNativeToken(
          signer,
      ...
      _txHash = await bridgeTokens(
        ...
        convertedAmount,
        selectedDstAddress || wallet?.accounts[0].address,
        selectedDestinationChainNetwork.chainId
      );
    } catch (error) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-6 pl-2">
          ...
          <div
            className={
              selectedSourceChainNetwork && selectedSourceToken
                ? ""
                : "disabled-modal"
            }
          >
        ...
        <div>
          {isInputVisible && (
            <div>
              <label>
                <div className="flex items-center justify-center p-1 mt-4">
                  <p className="text-sm font-bold w-56">Destination address:</p>
                  <Input
                    className="bg-[#E1E1FF] rounded-3xl p-2 text-sm"
                    value={selectedDstAddress}
